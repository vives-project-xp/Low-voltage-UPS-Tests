...
//mqtt publisher
static void mqtt_publish_task(char *message , char *message2)
{
    int message_id;
    message_id = esp_mqtt_client_publish(client, "homeassistant/battery/InUse", message, 0, 0, 0);
    message_id = esp_mqtt_client_publish(client, "homeassistant/battery/Status", message2, 0, 0, 0);
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        char message2[30];
        sprintf(message2, "%s", led_state == true ? "0" : "1");
        mqtt_publish_task(message, message2);
        //
